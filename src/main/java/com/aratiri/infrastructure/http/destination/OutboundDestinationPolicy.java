...
/**
 * Fail-closed outbound destination policy for user-influenced HTTP (webhooks, LNURL, NIP-05).
 *
 ...
 * rebinding. A non-empty {@code aratiri.outbound.destination.allowed-hosts} list narrows which names
...
public class OutboundDestinationPolicy {
  ...
  private final OutboundHostResolver hostResolver;

  public OutboundDestinationPolicy(
      OutboundDestinationProperties properties,
      OutboundHostResolver hostResolver) {
  ...
  private static URI parseAbsoluteUri(String rawUrl) {
    ...
    try {
      ...
      throw new OutboundDestinationRejectedException(e);
    }
  }
